...
- **用户配置统一入口**：动态菜单通过 `GET /user-config?key=menu` 按当前权限读取；全部可自定义列表的列显隐、顺序、宽度和固定位置保存到后台个人配置，挂载自动恢复，跨设备生效，支持重置默认。
- **菜单即时更新**：新增、修改和删除菜单后，在应用内同步侧边栏、路由和按钮权限，保留有效页面与标签状态，无需整页刷新。
...
- **安全邮箱找回密码**：已绑定时默认收起表单，点击「更换安全邮箱」后再填写；同屏填写邮箱、当前密码和验证码，验证码框右侧发送、独立按钮绑定；找回时校验图形验证码及已绑定邮箱验证码，Redis 原子防重放，重置后旧登录凭证失效，可复用现有邮箱 SMTP
...
| 模块 | 内容 |
|---|---|
| 认证 | 登录 / 双 Token 刷新 / 登出 / BCrypt 密码加密 / 凭证变更与强退后旧 token 即时失效（版本号机制）；返回登录自动清理未提交的密码与校验状态，保留记住账号功能；账号由管理员统一创建，不提供自助注册 |
| 登录安全 | 服务端图形验证码（一次性 + 2 分钟有效期）、失败锁定（用户名/IP 双维度 15 分钟 5 次）、声明式接口限流 `@RateLimit`——状态存 Redis，重启不丢、多实例共享 |
| 邮箱验证与找回 | 个人中心同屏发送验证码与独立按钮绑定安全邮箱（与联系邮箱隔离），已绑定时点击「更换安全邮箱」展开表单 / 账号 + 图形验证码发信 / 6 位邮箱验证码重置密码 / 5 分钟有效、5 次尝试、60 秒发送冷却；登录页仅保留账号密码、记住账号与忘记密码 |
| 防重复提交 | `@Idempotent` 注解 + Redis SETNX 原子占位：同用户在时间窗内重复提交返回 409（未登录降级 IP 维度），资料修改、文件上传等写接口已接入 |
| 系统管理 | 用户、角色、菜单、部门管理（CRUD + 树形结构 + 分页） |
| 权限 | 按钮级 RBAC 权限码、菜单驱动动态路由、角色授权；刷新或新窗口直达后台页面时先初始化菜单，个人中心与消息中心同样适用；菜单分组可同时展开，互不自动收起 |
...
| 个人列表配置 | 用户、角色、菜单、部门、字典类型、文件、模型配置、操作日志、登录日志、在线用户共 10 个可自定义列表：`POST /user-config/save` 提交 `{ key, value: JSON数组 }`，挂载时 `GET /user-config?key=...` 恢复；配置按用户和表格隔离，重置保存空数组 |
| 用户菜单配置 | `GET /user-config?key=menu` 返回当前登录用户有权访问的动态路由树，无菜单返回空数组；`menu` 为只读保留 key，不接受个人配置保存；菜单管理写入后应用内同步菜单、动态路由、标签与按钮权限，当前页面失效时自动跳转可访问页面 |
...
| 公告广播 | 管理员发布页（系统管理 → 公告发布，super/admin 可见）：按全员/部门（部门树多选）/指定用户（搜索多选）三粒度发布，复用站内通知通道落库 + 铃铛实时推送（`msg_type=announcement`，`POST /notice/announce` 挂 `Notice:Announce:Publish` 权限码 + 幂等防重，仅发送启用账号）；公告记录发布人登录名快照（`sys_notice.publisher`），消息中心公告列表展示发布人 |
...
- **25 个工具、注解零适配注册**：业务 Service 方法标 `@AiAgentTool`（名称/标题/类型/权限码/是否高危）即自动成为 AI 工具，启动时反射扫描注册、JSON Schema 由方法签名生成——新增工具不再需要改任何注册代码。覆盖：用户/角色/部门/菜单/字典/文件/在线用户/登录日志/操作日志/仪表盘查询，用户/角色/部门/公告创建，用户/角色/部门删除，密码重置，在线强退，角色菜单授权，IM 聊天（联系人/会话/聊天记录查询、发消息——发送走确认卡片）
...
- **菜单同步**：菜单保存后不再整页刷新；若读取新菜单失败，可点击“重新同步菜单”重试，无需重复保存。其他已登录设备在重新加载菜单时获取最新授权。

- **菜单接口迁移**：动态菜单读取统一为 `/api/user-config?key=menu`，原 `/api/menu/all` 已移除，前后端需同步更新；菜单管理及角色授权仍通过原管理接口维护。

- **列表配置升级**：存量库执行 `service/src/main/resources/db/upgrade-20260924-user-config-postgres.sql`（MySQL 使用同名 `-mysql.sql`），dev/docker 启动也会自动建表。新增可自定义列表必须通过业务 `#/adapter/vxe-table`，指定唯一且稳定的 `gridOptions.id`（如 `table.system.user`），普通列与操作列须有稳定 `field`，序号等类型列可使用 VXE 内置类型标识。配置只保存布局，不保存标题、行数据或函数；读取失败使用默认布局，保存失败按接口错误提示。

- **账号管理**：后台不开放自助注册，登录页无创建账号入口；账号由管理员在「系统管理 → 用户管理」统一创建和分配权限。登录仍使用一次性图形验证码；登录失败后自动关闭验证码弹窗，再次登录会获取新验证码。

- **安全邮箱**：在「个人中心 → 安全邮箱」填写目标邮箱与当前密码，点击验证码输入框右侧的「发送邮箱验证码」，收到邮件后填写验证码并点击「绑定」。已绑定时默认隐藏表单，点击「更换安全邮箱」后再进行更换；绑定或更换成功后自动收起表单。发信请求单独等待最多 60 秒，发送成功后有 60 秒重发冷却；修改邮箱或密码后需重新获取验证码。基本设置中的联系邮箱不会自动成为安全邮箱。未绑定或无法收信时由管理员重置；找回的新密码要求 8–64 位 ASCII 可见字符，含字母和数字、无空格。
- **SMTP 配置**：本项目使用标准 SMTP，不内置按条计费的供应商。可复用支持 SMTP 的现有邮箱，免费额度与费用以邮箱服务商为准。后端进程需配置 `SMTP_HOST`、`SMTP_PORT`、`SMTP_USERNAME`、`SMTP_PASSWORD`（邮箱服务商的 SMTP 授权码）和可选 `SMTP_FROM`（默认同用户名）。凭据只放部署环境或未跟踪的本地配置，不写入 Git。默认 587 + STARTTLS；使用 465 隐式 TLS 时设置 `SMTP_PORT=465`、`SMTP_SSL=true`、`SMTP_STARTTLS=false`。认证默认开启，只有无认证的本地测试邮件服务才设置 `SMTP_AUTH=false`。配置完成后重启后端；未配置时界面明确提示邮箱验证未开启，不伪造发信成功。
- **存量数据库升级**：安全邮箱存储于 `sys_user_recovery_email`，不迁移既有未验证邮箱。开发 PostgreSQL 启动时幂等建表；手工部署分别执行 `service/src/main/resources/db/upgrade-20260923-email-recovery-postgres.sql` 或 `upgrade-20260923-email-recovery-mysql.sql`。
- **邮件发送排查**：服务商明确返回收件账号不存在时，页面提示「邮箱不存在，请检查邮箱地址后重新发送」；其他错误区分 SMTP 认证失败、连接失败、响应超时、TLS 失败、收件地址被拒绝及服务商拒绝发送。后端 `Recovery email delivery failed` 日志只记录错误分类、异常类型和可用的 SMTP 状态码，不记录邮箱、授权码、正文或验证码。
- **集成测试**：常规 `mvn test` 不依赖本地数据库。显式设置 `RECOVERY_INTEGRATION_TEST=true` 可在本地开发 PostgreSQL/Redis 上验证真实 HTTP 绑定、找回、旧令牌失效和 Lua 验证码并发消费；邮件投递器被替换为测试实现，临时账号自动清理，不向真实邮箱发信。
