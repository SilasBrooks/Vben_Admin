...
const greeting = computed(() => {
  ...
  if (hour < 6) return $t('dashboard.workspace.greeting.dawn');
  if (hour < 12) return $t('dashboard.workspace.greeting.morning');
  if (hour < 18) return $t('dashboard.workspace.greeting.afternoon');
  return $t('dashboard.workspace.greeting.evening');
});

/** 今日日期（xxxx年x月x日 星期x） */
const todayText = computed(() => {
  const now = new Date();
  const week = $t('dashboard.workspace.weekdays').split(',')?.[now.getDay()];
  return $t('dashboard.workspace.date', {
    d: now.getDate(),
    m: now.getMonth() + 1,
    w: week,
    y: now.getFullYear(),
...
const statCards = computed(() => {
  ...
  return [
    { color: '#409eff', label: $t('dashboard.workspace.stat.user'), path: '/system/user', value: totals?.userCount ?? 0 },
    { color: '#67c23a', label: $t('dashboard.workspace.stat.role'), path: '/system/role', value: totals?.roleCount ?? 0 },
    { color: '#e6a23c', label: $t('dashboard.workspace.stat.dept'), path: '/system/dept', value: totals?.deptCount ?? 0 },
    { color: '#f56c6c', label: $t('dashboard.workspace.stat.online'), path: '/monitor/online', value: totals?.onlineCount ?? 0 },
    { color: '#9a66e4', label: $t('dashboard.workspace.stat.file'), path: '/system/file', value: totals?.fileCount ?? 0 },
    { color: '#36cfc9', label: $t('dashboard.workspace.stat.todayLogin'), path: '/monitor/login-log', value: summary.value?.today.loginSuccess ?? 0 },
  ];
});
...
<template>
  <Page>
      ...
      >
        ...
        <div>
          <div class="text-lg font-semibold">
            {{ $t('dashboard.workspace.welcome', { greeting, name: userStore.userInfo?.realName }) }}
          </div>
          <div class="mt-1 text-sm opacity-80">{{ $t('dashboard.workspace.today', { today: todayText }) }}</div>
        </div>
      </div>
    </ElCard>
    ...
    <div class="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-2">
      ...
      <ElCard shadow="never">
        <template #header>{{ $t('dashboard.workspace.recentLogins') }}</template>
        <ElTable :data="summary?.recentLogins ?? []">
          <ElTableColumn prop="username" :label="$t('dashboard.workspace.username')" width="120" />
